...
teacher_meeting_header = 'What type of event would you like to participate in?'
...
def meeting_filter(meeting):
    matched  = [
        True if meeting in matched_meeting else False
        ...
        if topic_matched.eq(False).all() and list_end == 1: # print when list end
            ...
            if location_matched.eq(False).all():
                ...
                if school_matched.eq(False).all():
                    ...
                    meeting_matched = meeting_filter(meeting)
                    if meeting_matched.eq(False).all():
                        unmatched_teacher.append(name_teacher) #
                        ...
                        matched_meeting.append(meeting)
                        priority.append(n + 1)
...
title1 = ['Teachers','Teacher_email','Teacher Topic', 'Priority Number', 'Common Topics','Physicists Topic', 'Physicists', 'Physicist_email', 'Month Matched',  'Type of Visit(T)', 'Time Zone(T)', 'School Level(T)', 'Type of Visit(P)', 'Time Zone(P)', 'School Level(P)', 'Teacher Physics/Science class level']
